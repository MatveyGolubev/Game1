...
void renderScene(void) {
    ...
    if (isGameOver) {
        ...
        // Перемещение камеры
        glTranslatef(-figur.pos.x, -figur.pos.y, 0);
        drawMap();
        //границы
...
void handleCollisions(void) {
    for (int i = 0; i < MAX_ENEMIES; ++i) {
        if (checkCollision(figur.pos, enemies[i].pos, 0.1, 0.1)) {
            ...
            figur.health -= 0.5; // Уменьшение здоровья персонажа
            if (figur.health <= 0) {
...
void update(int value) {
    ...
    if (figur.pos.y >= 0.001 || figur.pos.y <= -0.001) {
        moveUp = !moveUp;
    }
    //статичная для мобов
    ...
    for (int i = 0; i < MAX_ENEMIES; ++i) {
        ...
        if (enemies[i].pos.y >= 0.03 || enemies[i].pos.y <= -0.03) {
...
void drawKnives() {
    for (int i = 0; i < MAX_KNIVES; ++i) {
        if (knives[i].isAlive) {
            glPushMatrix();
            glTranslatef(knives[i].pos.x, knives[i].pos.y, 0);
            knife();
            glEnd();
            glPopMatrix();
        }
...
void drawGameOver(void) {
    ...
    glRasterPos2f(figur.pos.x,figur.pos.y);
...
void drawHealthBar(void) {
    glPushMatrix();
    glTranslatef(figur.pos.x, figur.pos.y+0.15, 0);
    glScalef(0.2, 0.2, 0.2);

    glBegin(GL_QUADS);
    glColor3f(1, 0, 0); //здоровье
    ...
    //значения health
    glVertex2f(-0.3, 0.2);
    glVertex2f(figur.health * 0.08 - 0.3, 0.2);
    glVertex2f(figur.health * 0.08 - 0.3, 0.25);
    glVertex2f(-0.3, 0.25);
...
void generateRandomEnemies(void) {          //случайная генерация мобов
    ...
    for (int i = 0; i < MAX_ENEMIES; ++i) {
        ...
        do {
            ...
            for (int j = 0; j < i; ++j) {
                if (checkCollision({posX, posY}, enemies[j].pos, 0.4, 0.4)) {
                    overlap = true;
                    break;
                }
            }
